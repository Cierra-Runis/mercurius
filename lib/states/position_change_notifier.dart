...
class PositionModel extends ChangeNotifier {
  ...
  Future<void> _getPosition(bool force) async {
    if (profileModel.profile.cache.cachePosition != null && !force) {
      cachePosition = profileModel.profile.cache.cachePosition!;
    } else {
      Cache newCache = profileModel.profile.cache;
      cachePosition = await _getAMap();
      profileModel.changeProfile(
        profileModel.profile..cache = (newCache..cachePosition = cachePosition),
      );
    }
    notifyListeners();
  ...
  void init() {
    DevTools.printLog('PositionModel 初始化中');
    update(false);
    notifyListeners();
    super.notifyListeners();
  }
